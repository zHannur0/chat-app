...
export async function GET(req: NextRequest) {
  try {
    ...
    let users = snaps.docs.map(d => ({ uid: d.id, ...(d.data() as any) }));
      ...
      if (!u.displayName || !u.displayNameLower || !u.photoURL) {
        try {
          ...
          if (displayName || photoURL) {
            users[idx] = { ...u, displayName, displayNameLower: (displayName || '').toLowerCase(), photoURL };
            await getFirestore().collection('users').doc(u.uid).set({
              displayName,
    ...
    const nextCursor = users.length ? users[users.length - 1]?.emailLower : undefined;
