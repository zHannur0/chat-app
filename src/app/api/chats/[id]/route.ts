...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const auth = await verifyBearerToken(_req.headers.get('authorization') || undefined);
    const chat = await getChatById(params.id);
    if (!chat) return NextResponse.json({ error: 'not found' }, { status: 404 });

    let peer: { uid: string; displayName?: string; photoURL?: string; username?: string } | undefined;
    if (chat.type === 'direct') {
      const peerId = chat.memberIds.find(id => id !== auth.uid);
      if (peerId) {
        const uDoc = await usersCollection().doc(peerId).get();
        let u = uDoc.data() as any | undefined;
      ...
      peer = { uid: chat.botId || 'bot:assistant', displayName: process.env.BOT_NAME || 'AI Bot' };
    }
