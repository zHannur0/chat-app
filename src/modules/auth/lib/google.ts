      ...
      const prompt = (label: string) => {
        try {
          window.google!.accounts.id.prompt((notification: any) => {
            try { console.log('[auth] one-tap moment', label, notification.getMomentType?.(), notification.getDismissedReason?.(), notification.getSkippedReason?.()); } catch {}
      ...
      window.setTimeout(() => {
        ...
        window.setTimeout(() => {
          ...
          try {
            ...
            if (btn) btn.click();
            window.setTimeout(() => {
              if (!settled) finish(undefined, new Error('Google One Tap timeout'));
            }, 4000);
          } catch (e) {
...
export async function beginGoogleRedirectSignIn(navigatePath: string): Promise<void> {
  await loadGoogleScript();
  const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
  if (!clientId) throw new Error('Missing NEXT_PUBLIC_GOOGLE_CLIENT_ID');
  window.google!.accounts.id.initialize({
    ...
    callback: (credentialResponse: any) => {
      if (credentialResponse?.credential && typeof window !== 'undefined') {
        try { localStorage.setItem('google_redirect_credential', credentialResponse.credential as string); } catch {}
  ...
  window.google!.accounts.id.prompt(undefined, { use_fedcm_for_prompt: true, itp_support: true });
}

// On the redirect destination page, await the Google credential delivered by GIS
export async function awaitGoogleRedirectCredential(timeoutMs = 10000): Promise<string | undefined> {
  await loadGoogleScript();
  const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
  ...
  return new Promise<string | undefined>((resolve) => {
    ...
    setTimeout(() => { if (!settled) resolve(undefined); }, timeoutMs);
  });
}
