...
import { messageQueue, QueuedMessage, MessageStatus } from '@/modules/chat/lib/messageQueue';
...
export function useSendMessage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const toRetry = queue.filter(item => 
        item.status === 'failed' && item.nextRetry <= now
      );
  ...
  const retryMessage = useCallback(async (item: QueuedMessage) => {
    try {
      await sendMessage({ 
        chatId: item.chatId, 
        text: item.text, 
  ...
  const sendMessageWithRetry = useCallback(async (
    ...
    try {
      ...
      const updatedMessage = { ...queuedMessage, status: 'sent' as MessageStatus };
      messageQueue.updateMessage(clientId, updatedMessage);
      setQueue(prev => prev.map(q => 
        q.id === clientId ? updatedMessage : q
      ));
