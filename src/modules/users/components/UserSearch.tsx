...
import { useCreateChatMutation, useListChatsQuery } from '@/modules/chat/api/chatApi';
...
const UserSearch = ({ onChatCreated }: Props) => {
    ...
    const getExistingChatId = (uid: string): string | undefined => {
        ...
        const found = list.find(c => c.type === 'direct' && Array.isArray(c.memberIds) && c.memberIds.includes(uid));
        return found?.id;
    };
                    ...
                    {data?.users?.map(u => {
                        ...
                        return (
                            <div key={u.uid} className="flex items-center justify-between">
                                <div className="truncate">{u.displayName || u.displayName || u.email || u.uid}</div>
                                {existing ? (
                                    <button className="text-primary" onClick={() => router.push(`/chat?chat=${encodeURIComponent(existing)}`)}>Open</button>
                                ) : (
